...
namespace KNNRegression
{
  internal class KNNRegressionProgram
  {
    public static void MainKNN(string[] args)
    {
      ...
      // 5. create and train/fit model
      Console.WriteLine("\nCreating k-NN model, k=5 ");
      KNNR knnr5 = new KNNR(5);
      knnr5.Train(trainX, trainY);

      // 6. use a new model
      Console.WriteLine("\nPredicting for runtime sample:" +
        " [0.7462, 0.4006, -0.0590, 0.6543, -0.0083]");
      x = new double[] { 0.7462, 0.4006, -0.0590, 0.6543, -0.0083 };
      y = knnr5.Predict(x);
      Console.WriteLine("y = " + y.ToString("F4").PadLeft(8));
  ...
  public class KNNR
  {
    ...
    public double Predict(double[] x)
    {
      ...
      {
        int idx = sortedIdxs[k];
        double neighborY = this.trainY[idx];
        sum += neighborY;
      }
      return sum / this.nNeighbors;
    }
